...
function checkMath(callback=0, num0,num9,...numx) {
  let opResult = num0 + num9 + numx.reduce((x,y)=>{return x+y});
...
function resetQuestion() {
  generateNumbers();
  if (selectedOperator == 3) {
    document.querySelectorAll("#question")[0].innerHTML =
    `${num1}&#160;${mathOperators[selectedOperator].sign}&#160;${Math.round(Math.sqrt(num2))}&#160;=`;
  } else {
    document.querySelectorAll("#question")[0].innerHTML =
    `${num1}&#160;${mathOperators[selectedOperator].sign}&#160;${num2}&#160;=`;
  }
}
...
function clearBtn() {
  ...
  selectedOperator = Math.floor(Math.random() * mathOperators.length);
  resetQuestion();
}
